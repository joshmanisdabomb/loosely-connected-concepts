...
class LandmineFeature(configCodec: Codec<DefaultFeatureConfig>) : Feature<DefaultFeatureConfig>(configCodec) {
    ...
    override fun generate(context: FeatureContext<DefaultFeatureConfig>): Boolean {
        with (context) {
            if (!GenUtils.areaMatches(world::getBlockState, origin.x, origin.y - 3, origin.z, height = 3) { state, pos -> !world.isOutOfHeightLimit(pos.y) }) return false
            ...
            if (!GenUtils.areaMatches(world::getBlockState, origin.x, origin.y - 3, origin.z, ex = 1, height = 1, ez = 1) { state, pos -> state.isSolidBlock(world, pos) }) return false
