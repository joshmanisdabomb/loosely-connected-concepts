...
public class FunctionalSnowlessGrassBlock extends Block implements LCCBlockHelper {
    ...
    public FunctionalSnowlessGrassBlock(Function<BlockState, BlockState> change, Properties properties) {
        super(properties);
        this.change = change;
    }
    ...
    public void tick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!world.isRemote) {
            ...
            if (!survives(state, world, pos, false)) {
                world.setBlockState(pos, this.change.apply(null));
            } else {
                    ...
                    for(int i = 0; i < 4; ++i) {
                        ...
                        if (survives(state, world, pos2, true)) {
                            ...
                            if (param.getBlock() != this) {
                                BlockState set = this.change.apply(param);
