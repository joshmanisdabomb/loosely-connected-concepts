...
class ComputingBlock(settings: Settings) : BlockWithEntity(settings), LCCBlockTrait, SubblockSystem {
    ...
    override fun getPlacementState(ctx: ItemPlacementContext): BlockState {
        ...
        if (state.block == this) {
            (ctx.world.getBlockEntity(ctx.blockPos) as? ComputingBlockEntity)?.setHalf(item.getComputingHalf(ctx.stack, ctx.playerFacing.opposite, state[SLAB_TYPE] != SlabType.TOP))
    ...
    override fun onPlaced(world: World, pos: BlockPos, state: BlockState, placer: LivingEntity?, stack: ItemStack) {
        ...
        if (state[SLAB_TYPE] != SlabType.DOUBLE) {
            ...
            (world.getBlockEntity(pos) as? ComputingBlockEntity)?.setHalf(item.getComputingHalf(stack, placer?.horizontalFacing?.opposite ?: Direction.SOUTH, top))
        }
