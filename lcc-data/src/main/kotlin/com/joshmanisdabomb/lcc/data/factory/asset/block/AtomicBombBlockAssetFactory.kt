        ...
        stateVariant(data, entry) {
            coordinate(BlockStateVariantMap.create(AtomicBombBlock.segment, Properties.HORIZONTAL_FACING).register { segment, dir ->
                BlockStateVariant.create().put(VariantSettings.MODEL, when (segment) {
                    AtomicBombBlock.AtomicBombSegment.HEAD -> ModelTemplates.template_atomic_bomb_head.upload(suffix(id, "head"), Texture().put(LCCModelTextureKeys.t2, suffix(id, "tail")).put(LCCModelTextureKeys.t5, suffix(id, "head")).put(TextureKey.PARTICLE, suffix(id, "tail")), data.modelStates::addModel)
                    AtomicBombBlock.AtomicBombSegment.TAIL -> ModelTemplates.template_atomic_bomb_tail.upload(suffix(id, "tail"), Texture().put(LCCModelTextureKeys.t1, suffix(id, "tail_side")).put(LCCModelTextureKeys.t2, suffix(id, "tail")).put(LCCModelTextureKeys.t3, suffix(id, "fin")).put(LCCModelTextureKeys.t4, suffix(id, "core")).put(TextureKey.PARTICLE, suffix(id, "tail")), data.modelStates::addModel)
                    else -> ModelTemplates.template_atomic_bomb_middle.upload(suffix(id, "middle"), Texture().put(LCCModelTextureKeys.t2, suffix(id, "tail")).put(LCCModelTextureKeys.t5, suffix(id, "head")).put(TextureKey.PARTICLE, suffix(id, "tail")), data.modelStates::addModel)
