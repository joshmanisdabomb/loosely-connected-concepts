...
open class StaticFireBlockAssetFactory(val texture: Identifier? = null) : BlockAssetFactory {
    ...
    override fun apply(data: DataAccessor, entry: Block) {
        ...
        val floors = (0..1).map { Models.TEMPLATE_FIRE_FLOOR.upload(idh.locSuffix(entry, "floor$it"), TextureMap().put(TextureKey.FIRE, t.suffix(it.toString())), data.models) }
        val sides = (0..1).map { Models.TEMPLATE_FIRE_SIDE.upload(idh.locSuffix(entry, "side$it"), TextureMap().put(TextureKey.FIRE, t.suffix(it.toString())), data.models) }
        val sideAlts = (0..1).map { Models.TEMPLATE_FIRE_SIDE_ALT.upload(idh.locSuffix(entry, "side_alt$it"), TextureMap().put(TextureKey.FIRE, t.suffix(it.toString())), data.models) }
