...
open class ConnectedTextureModel(defaultPrefix: String, val connector: (world: BlockView, state: BlockState, pos: BlockPos, other: BlockState, otherPos: BlockPos, path: Array<Direction>) -> Boolean = { world, state, pos, other, otherPos, path -> state == other }, val borderSize: Int = 4, val pos1: Vec3f = Vec3f(0f, 0f, 0f), val pos2: Vec3f = Vec3f(1f, 1f, 1f), val innerSeams: Boolean = true, mapConsumer: ConnectedTextureMap.() -> Unit = {}) : LCCModel({ (this as ConnectedTextureModel).textureMap }) {
    ...
    inner class ConnectedTextureMap internal constructor() {
        ...
        private val keyCache = mutableMapOf<Pair<Direction?, Int>, String>()

        fun set(prefix: String, vararg faces: Direction?, on: (index: Int) -> Boolean = { true }): ConnectedTextureMap {
            this.sets += prefix
        ...
        fun top(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, *mutableListOf<Direction?>(Direction.UP).apply { if (particle) this.add(null) }.toTypedArray(), on = on)
        fun bottom(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, *mutableListOf<Direction?>(Direction.DOWN).apply { if (particle) this.add(null) }.toTypedArray(), on = on)
        fun north(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, *mutableListOf<Direction?>(Direction.NORTH).apply { if (particle) this.add(null) }.toTypedArray(), on = on)
        fun east(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, *mutableListOf<Direction?>(Direction.EAST).apply { if (particle) this.add(null) }.toTypedArray(), on = on)
        fun south(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, *mutableListOf<Direction?>(Direction.SOUTH).apply { if (particle) this.add(null) }.toTypedArray(), on = on)
        fun west(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, *mutableListOf<Direction?>(Direction.WEST).apply { if (particle) this.add(null) }.toTypedArray(), on = on)

        fun sides(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, Direction.NORTH, Direction.EAST, Direction.SOUTH, Direction.WEST, on = on)
        fun all(prefix: String, particle: Boolean = false, on: (index: Int) -> Boolean = { true }) = set(prefix, null, *Direction.values(), on = on)

        fun particle(texture: String, on: (index: Int) -> Boolean = { true }) = this.also { particles.add(texture); particlePredicates.add(on) }
        ...
        fun getKey(face: Direction?, segment: String? = if (face == null) null else "base", index: Int = 0): String {
            val key = keyCache.getOrPut(face to index, when (face) {
                null -> { { particles.filterIndexed { i, s -> particlePredicates[i](index) }.lastOrNull() ?: sets.filterIndexed { i, s -> faces[i].contains(face) && predicates[i](index) }.map { "${it}_${segment ?: "base"}" }.last() } }
                else -> { { sets.filterIndexed { i, s -> faces[i].contains(face) && predicates[i](index) }.last() } }
            })
            return if (face == null) key else key.plus("_").plus(segment!!)
        }
        ...
        val map by lazy {
            sets.distinct().flatMap { s -> segments.map { "${s}_$it" to SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, LCC.id("block/${s}_$it")) } }.toMap().plus(particles.distinct().map { it to SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, LCC.id("block/$it")) })
        }
