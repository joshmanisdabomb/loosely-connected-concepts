...
object LCCKnowledgeData: BasicDirectory<KnowledgeArticleBuilder, Unit>() {
    ...
    val lcc_fabric_0_5_0 by entry(::initialiser) {
        KnowledgeArticleBuilder(LCCVersion.LCC_FABRIC_0_5_0.page)
            .translation("LCC 0.5.0")
            .addSection(KnowledgeArticleSectionBuilder(introduction)
                .addFragment(KnowledgeArticleTextFragmentBuilder(LCCVersion.LCC_FABRIC_0_5_0.getIntroduction())
                    .insert(LiteralText("Loosely Connected Concepts 0.5.0"))
                    .insert(LiteralText("fifteenth"))
                    .insert(LiteralText("58th"))
                    .insertLink(LiteralText("LCC 0.4.4"), LCCVersion.LCC_FABRIC_0_4_4.page)
                )
                .addFragment(KnowledgeArticleTextFragmentBuilder("The focus of LCC 0.5.x is the %s biome, a hostile and dismal expanse filled with vicious traps and difficult mobs.")
                    .insertLink(LiteralText("Wasteland"), KnowledgeArticleIdentifier(BuiltinRegistries.BIOME.key.value, LCC.id("wasteland")))
                )
            )
            .addSection(KnowledgeArticleSectionBuilder(LiteralText("Gameplay"))
                .addFragment(KnowledgeArticleTextFragmentBuilder("In this version, the Wasteland biome is split into two sub-biomes, %s and %s. The barrens are flat terrain where traps, resources and larger quantities of mobs are commonly found. The spiked regions are generally safer, but provide less rewards and are challenging to navigate.")
                    .insertLink(LiteralText("Wasteland Barrens"), KnowledgeArticleIdentifier.ofBiome(LCCBiomes.wasteland_barrens))
                    .insertLink(LiteralText("Wasteland Spikes"), KnowledgeArticleIdentifier.ofBiome(LCCBiomes.wasteland_spikes))
                )
                .addFragment(KnowledgeArticleTextFragmentBuilder("The new biome provides a brand new challenge to the player, as a specific tool progression will need to be followed in the Wasteland biome. Some blocks require the use of a tool from the Wasteland to mine, and most mobs take reduced damage from non-Wasteland equipment and damage through non-Wasteland armor. The progression starts at %s, a new wood type which can be found in clusters scattered across the barrens. A %s is required to mine %s which appears on the surface in the spikes sub-biome. Players must then convert their blocks of iron into %s by submerging them in water in any Wasteland sub-biome. Finally, players must complete challenges posed by %s structures to obtain %s, the current final tier.")
                    .insertLink(LiteralText("Wasteland Barrens"), KnowledgeArticleIdentifier.ofBiome(LCCBiomes.wasteland_barrens))
                    .insertLink(LiteralText("Wasteland Spikes"), KnowledgeArticleIdentifier.ofBiome(LCCBiomes.wasteland_spikes))
                    ...
                    .insertLink(LiteralText("rusted iron blocks"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.rusted_iron_blocks.values.last()))
                    .insertLink(LiteralText("Sapphire Altars"), KnowledgeArticleIdentifier.ofStructure(LCCStructureFeatures.sapphire_altar))
                    .insertLink(LiteralText("sapphires"), KnowledgeArticleIdentifier.ofItem(LCCItems.sapphire))
                )
                .addFragment(KnowledgeArticleTextFragmentBuilder("As well as new gear, players can also find treasure in the Wasteland. %s are structures that spawn in the barrens which often contain chests containing loot. Obtained items can range from sticks to %s, which can permanently increase your maximum life. '%s' can also be found in the barrens, grouped in clusters. Breaking these will sometimes drop gold pieces, iron pieces or the aforementioned heart containers.")
                    .insertLink(LiteralText("Tents"), KnowledgeArticleIdentifier.ofStructure(LCCStructureFeatures.wasteland_tent))
                    .insertLink(LiteralText("heart containers"), KnowledgeArticleIdentifier(LCC.id("concepts"), LCC.id("heart_containers")))
                    .insertLink(LiteralText("Deposits"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.deposit))
                )
                .addFragment(KnowledgeArticleTextFragmentBuilder("The Wasteland is now home to three dangerous mobs. The %s is a baby version of a regular Minecraft skeleton that strafes and shoots faster and is harder to hit. %s are short creatures that extends its tongue out to latch on to its prey. Finally, %s nests spawn in all parts of the Wasteland. Wasps are generally neutral but will vehemently defend its nest from perceived threats and can often get randomly aggressive at nearby targets.")
                    .insertLink(LCCEntities.baby_skeleton.name, KnowledgeArticleIdentifier.ofEntity(LCCEntities.baby_skeleton))
                    .insertLink(LiteralText("Consumers"), KnowledgeArticleIdentifier.ofEntity(LCCEntities.consumer))
                    .insertLink(LCCEntities.wasp.name, KnowledgeArticleIdentifier.ofEntity(LCCEntities.wasp))
                )
                .addFragment(KnowledgeArticleTextFragmentBuilder("%s traps and landmines hooked up to camouflaged %s are often placed in Wasteland Barrens. Spikes can sometimes spawn covered in %s or %s residue. Landmines are rigged up with an %s, which leaves behind a powerful firey explosion.")
                    .insertLink(LiteralText("Spike"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.spikes))
                    .insertLink(LiteralText("pressure plates"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.cracked_mud_pressure_plate))
                    .insertLink(LiteralText("bloody"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.bleeding_spikes))
                    .insertLink(LiteralText("poisonous"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.poison_spikes))
                    .insertLink(LCCBlocks.improvised_explosive.name, KnowledgeArticleIdentifier.ofBlock(LCCBlocks.improvised_explosive))
                )
                .addFragment(KnowledgeArticleTextFragmentBuilder("Oil refining has also been modified in this version. Oil, which now spawns more commonly in the wasteland buried under %s and as geysers, now refines into three products: %s, %s and %s. Fuel can be converted to energy using a %s, or refined further into %s which can be placed to chain explosions, create improvised explosives (landmines) or to %s. Refined oil can be further refined into %s or %s, both of which can be customised with dyes and do not despawn. Flexible plastic can be crafted into %s, which hold 2 stacks of items. Tar balls are used to create %s.")
                    .insertLink(LiteralText("cracked mud"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.cracked_mud))
                    .insertLink(LiteralText("Fuel Buckets"), KnowledgeArticleIdentifier.ofItem(LCCItems.fuel_bucket))
                    .insertLink(LiteralText("Refined Oil Buckets"), KnowledgeArticleIdentifier.ofItem(LCCItems.refined_oil_bucket))
                    .insertLink(LiteralText("tar balls"), KnowledgeArticleIdentifier.ofItem(LCCItems.tar_ball))
                    ...
                    .insertLink(LiteralText("play a game of Minesweeper"), KnowledgeArticleIdentifier.of(LCCRegistries.altar_challenges, LCCAltarChallenges.minesweeper))
                    ...
                    .insertLink(LiteralText("plastic bags"), KnowledgeArticleIdentifier.ofItem(LCCItems.plastic_bag))
                    .insertLink(LiteralText("roads"), KnowledgeArticleIdentifier.ofBlock(LCCBlocks.road))
                )
            )
            .addSection(KnowledgeArticleSectionBuilder(LiteralText("Technical"))
                .addFragment(KnowledgeArticleTextFragmentBuilder("Similar to update %s, a lot of technical changes happened behind the scenes in addition to updated gameplay changes. The lcc-base module was split into lcc-infra and lcc-hooks. The infrastructure module provides helpers to other modules without making any changes to Minecraft with mixins and access transformers. The hooks module makes those kind of changes and provides other modules a way to interact with these hooks, using trait interfaces or accessor mixins.")
                    .insertLink(LiteralText("0.2.0"), LCCVersion.LCC_FABRIC_0_2_0.page)
                )
            ...
            .addSection(KnowledgeArticleSectionBuilder(LiteralText("Commit Messages from BitBucket"))
