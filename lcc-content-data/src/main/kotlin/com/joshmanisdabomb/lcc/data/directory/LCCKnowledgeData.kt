        ...
        KnowledgeArticleBuilder(LCCBlocks.sapphire_altar)
            .addSection(KnowledgeArticleSectionBuilder(introduction)
                .addFragment(KnowledgeArticleTextFragmentBuilder("%s is a block introduced in %s that allows players to interface with the challenge of a %s. It breaks into %s, even with the Silk Touch enchantment, making this block obtainable only in Creative Mode.")
                    ...
                    .insert/*Link*/("Sapphire Altar (Structure)"/*, KnowledgeArticleIdentifier.ofStructure(LCCStructureFeatures.sapphire_altar).link*/)
    ...
    val entity_consumer by entry(::initialiser) {
        KnowledgeArticleBuilder(LCCEntities.consumer)
            ...
            .addSection(KnowledgeArticleSectionBuilder("Drops")
                ...
                .addFragment(KnowledgeArticleLootFragmentBuilder(KnowledgeArticleTextFragmentBuilder("Can only drop when the %s's tongue is extended.").insert(LCCEntities.consumer.name)) { listOf(it.da.lootTables[LCCEntities.consumer_tongue]!!) }
                )
            )
            .addSection(KnowledgeArticleChangelogSectionBuilder())
    ...
    val block_mud by entry(::initialiser) {
        KnowledgeArticleBuilder(LCCBlocks.mud)
            ...
            .addSection(KnowledgeArticleSectionBuilder("Movement")
                ...
                .addFragment(KnowledgeArticleTextFragmentBuilder("Previously in %s, %s was a ridiculously slippery block, allowing entities walking on it to wildly accelerate and travel huge distances very quickly.")
