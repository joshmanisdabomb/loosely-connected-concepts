...
object LCCBlockData : BasicDirectory<BlockDataContainer, Unit>() {
    ...
    val rubber_leaves by entry(::initialiser) { BlockDataContainer().defaultLang().defaultItemAsset().add(LeavesBlockLootFactory(LCCBlocks.rubber_sapling, saplingChance = floatArrayOf(0.09F, 0.12F, 0.15F, 0.2F))).add(BlockTagFactory(BlockTags.LEAVES)).add(TintBlockAssetFactory).add(BlockTagFactory(BlockTags.LEAVES)).add(ItemTagFactory(ItemTags.LEAVES)) }
    ...
    val rubber_slab by entry(::initialiser) { BlockDataContainer().defaultLang().defaultItemAsset().defaultLootTable().add(SlabBlockAssetFactory(LCC.id("block/rubber_planks")) { d, b -> loc(LCCBlocks.rubber_planks) }).add(BlockTagFactory(BlockTags.WOODEN_SLABS)).add(ItemTagFactory(ItemTags.WOODEN_SLABS)).add(SlabRecipeFactory(LCCBlocks.rubber_planks)) }
    ...
    val treetap by entry(::initialiser) { BlockDataContainer().defaultLang().defaultItemAsset().add(TreetapBlockAssetFactory).add(CustomRecipeFactory { d, i ->
        ShapedRecipeJsonFactory(i, 1)
            ...
            .pattern("iii")
            .input('i', Items.IRON_INGOT)
            .apply { hasCriterionShaped(this, Items.IRON_INGOT) }
            .apply { offerShaped(this, d) }
