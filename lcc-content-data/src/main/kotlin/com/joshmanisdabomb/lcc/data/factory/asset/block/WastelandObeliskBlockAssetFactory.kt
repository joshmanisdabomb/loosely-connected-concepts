    ...
    override fun apply(data: DataAccessor, entry: Block) {
        stateVariant(data, entry) {
            coordinate(BlockStateVariantMap.create(WastelandObeliskBlock.charge, Properties.BOTTOM).register { charge, bottom ->
                val texture = TextureMap().put(TextureKey.TEXTURE, idh.loc(LCCBlocks.cracked_mud)).put(TextureKey.LANTERN, idh.locSuffix(entry, charge.toString())).put(TextureKey.INSIDE, idh.loc(entry)).put(TextureKey.PARTICLE, idh.loc(LCCBlocks.cracked_mud))
