...
interface GauntletProgressRenderer {
    ...
    fun renderLine(matrix: MatrixStack, camera: PlayerEntity, percentage: (entity: PlayerEntity, offset: Int) -> Double?, reverse: Boolean, u: Int, v: Int, angle: Float, delta: Float) {
        if (reverse) {
            renderLine(matrix, lerp(delta.toDouble(), 1 - (percentage(camera, -1) ?: 1.0), 1 - (percentage(camera, 0) ?: 1.0)), u, v, angle)
        } else {
            renderLine(matrix, lerp(delta.toDouble(), percentage(camera, -1) ?: 0.0, percentage(camera, 0) ?: 0.0), u, v, angle)
        }
    }
