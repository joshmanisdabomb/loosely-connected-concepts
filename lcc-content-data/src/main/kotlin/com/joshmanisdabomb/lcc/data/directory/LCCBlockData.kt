...
object LCCBlockData : BasicDirectory<BlockDataContainer, Unit>(), ModelAccess {
    ...
    val atomic_bomb by entry(::initialiser) {
        data().defaultLang().add(AtomicBombBlockAssetFactory).add(AtomicBombBlockLootFactory).add(CustomItemAssetFactory { d, t, i ->
            LCCModelTemplates.template_atomic_bomb_item.upload(idi.loc(t), Texture()
                ...
                .put(LCCModelTextureKeys.t5, idi.locSuffix(t, "head", folder = "block")), d.modelStates::addModel)
        })
            .add(CustomRecipeFactory { d, i ->
                ShapedRecipeJsonFactory(i, 1)
                    .pattern("ccc")
    ...
    val nuclear_generator by entry(::initialiser) {
        data().defaultLang().defaultLootTable().add(NuclearFiredGeneratorBlockAssetFactory).add(CustomItemAssetFactory { d, t, i ->
            LCCModelTemplates.template_nuclear_generator_item.upload(idi.loc(t), Texture()
                ...
                .put(LCCModelTextureKeys.t9, LCCBlocks.heavy_uranium_shielding.identifierLoc(folder = "block")), d.modelStates::addModel)
        }
        ).add(CustomRecipeFactory { d, i ->
            ShapedRecipeJsonFactory.create(i)
                .pattern("ses")
    ...
    val rhyolite_wall by entry(::initialiser) { data().defaultLang().defaultItemAsset().defaultLootTable().add(WallBlockAssetFactory(LCCBlocks.rhyolite.identifierLoc())).add(CustomItemAssetFactory { d, t, i -> Models.WALL_INVENTORY.upload(idi.loc(t), Texture().put(TextureKey.WALL, LCCBlocks.rhyolite.identifierLoc()), d.modelStates::addModel) }).add(BlockTagFactory(BlockTags.WALLS)).add(ItemTagFactory(ItemTags.WALLS)).add(WallRecipeFactory(LCCBlocks.rhyolite)).add(StonecutterItemRecipeFactory(LCCBlocks.rhyolite)) }
    ...
    val wasteland_obelisk by entry(::initialiser) {
        data().defaultLang().add(WastelandObeliskBlockAssetFactory).add(WastelandObeliskBlockLootFactory).add(CustomItemAssetFactory { d, t, i ->
            LCCModelTemplates.template_obelisk_item.upload(idi.loc(t), Texture()
                ...
                .put(TextureKey.LANTERN, idi.locSuffix(t, "0", folder = "block")), d.modelStates::addModel)
        }).add(CustomRecipeFactory { d, i ->
            ShapedRecipeJsonFactory.create(i)
                .pattern("cccc")
                .pattern(" tt ")
