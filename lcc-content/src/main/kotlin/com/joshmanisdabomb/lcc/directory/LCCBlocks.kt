...
object LCCBlocks : BlockDirectory() {
    ...
    //Computing
    val computing by entry(::initialiser) { ComputingBlock(FabricBlockSettings.of(Material.METAL, MapColor.WHITE).requiresTool().strength(3.0F, 0.0F).sounds(BlockSoundGroup.METAL)) }
        .setProperties(BlockExtraSettings().creativeEx(COMPUTING).cutout().blockColor(ComputingBlock::getTintColor))
    val terminal by entry(::initialiser) { TerminalBlock(FabricBlockSettings.of(Material.METAL, MapColor.WHITE).requiresTool().strength(3.0F, 0.0F).sounds(BlockSoundGroup.METAL)) }
