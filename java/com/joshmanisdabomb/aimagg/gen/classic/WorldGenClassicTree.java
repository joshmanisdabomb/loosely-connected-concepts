...
public class WorldGenClassicTree extends WorldGenerator {
    ...
    public WorldGenClassicTree(boolean notify) {
        super(notify);
    }
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getHeight()) {
			...
			if (!flag) {
				...
				if (state.getBlock().canSustainPlant(state, worldIn, position.down(), net.minecraft.util.EnumFacing.UP, ((IPlantable)AimaggBlocks.classicSapling)) && position.getY() < worldIn.getHeight() - i - 1) {
					...
					for (int j3 = 0; j3 < i; ++j3) {
						...
						if (state.getBlock().isAir(state, worldIn, upN) || state.getBlock().isLeaves(state, worldIn, upN) || state.getMaterial() == Material.VINE) {
							this.setBlockAndNotifyAdequately(worldIn, position.up(j3), LOG);
						}
					}
