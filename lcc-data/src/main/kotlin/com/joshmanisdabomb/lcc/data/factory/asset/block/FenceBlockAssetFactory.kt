...
class FenceBlockAssetFactory(val texture: Identifier? = null) : BlockAssetFactory {
    ...
    override fun apply(data: DataAccessor, entry: Block) {
        val post = Models.FENCE_POST.upload(loc(entry), Texture.texture(texture ?: loc(entry)), data.modelStates::addModel)
        val side = Models.FENCE_SIDE.upload(loc(entry) { it.plus("_side") }, Texture.texture(texture ?: loc(entry)), data.modelStates::addModel)
