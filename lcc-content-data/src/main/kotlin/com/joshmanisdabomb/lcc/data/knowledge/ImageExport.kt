...
class ImageExport(items: List<ItemConvertible>, entities: List<EntityType<*>>, val folder: File, val test: Long? = null) : Screen(TranslatableText("narrator.screen.title")) {
    ...
    private object NetworkHandler : ClientPlayNetworkHandler(MinecraftClient.getInstance(), null, ClientConnection(NetworkSide.CLIENTBOUND), MinecraftClient.getInstance().session.profile) {

    }

    private object World : ClientWorld(NetworkHandler, Properties(Difficulty.EASY, false, false), null, DimensionType.create(OptionalLong.empty(), true, false, false, false, 1.0, false, false, false, false, false, -16, 32, 32, VoronoiBiomeAccessType.INSTANCE, BlockTags.INFINIBURN_OVERWORLD.id, OVERWORLD_ID, 1f), 0, { null }, null, false, 0L) {
