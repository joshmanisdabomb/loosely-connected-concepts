...
public class ItemAntispreaderRemote extends ItemGeneric {

	private int power;

	public ItemAntispreaderRemote(String texture, int power, int durability) {
		super("remote/antispreader/" + texture);
		this.power = power;
			...
			for (int i = -power/2; i <= power/2; i++) {
	        	for (int j = -power/2; j <= power/2; j++) {
	        		for (int k = -power/2; k <= power/2; k++) {
	        	        ...
	        	        if (block == YetAnotherMod.antispreader) {
