...
class RecipeData(val batch: RecipeBatch, val da: DataAccessor) : DataProvider {
    ...
    override fun run(cache: DataCache) {
        batch.getRecipes().forEach {
            RecipeProviderAccessor.saveRecipe(cache, it.toJson(), da.path.resolve("data/" + it.recipeId.namespace + "/recipes/" + it.recipeId.path + ".json"))
            val json = it.toAdvancementJson() ?: return@forEach
            RecipeProviderAccessor.saveRecipeAdvancement(cache, json, da.path.resolve("data/" + it.advancementId!!.namespace + "/advancements/" + it.advancementId!!.path + ".json"))
        }
    }
