    ...
    override fun apply(data: DataAccessor, entry: Block) {
        stateVariant(data, entry) {
            coordinate(BlockStateVariantMap.create(DungeonTableBlock.ENTITY, Properties.BOTTOM).register { entity, bottom ->
                ...
                else BlockStateVariant.create().put(VariantSettings.MODEL, ModelTemplates.template_spawner_table.upload(loc(entry), Texture.sideTopBottom(entry).put(TextureKey.PARTICLE, loc(entry) { it.plus("_bottom") }), data.modelStates::addModel))
            })
        }
    }
