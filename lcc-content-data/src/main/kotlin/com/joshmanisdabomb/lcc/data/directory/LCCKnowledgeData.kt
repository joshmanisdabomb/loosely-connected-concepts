...
object LCCKnowledgeData : BasicDirectory<KnowledgeArticleBuilder, Unit>() {
    ...
    val block_cracked_mud by entry(::initialiser) {
        KnowledgeArticleBuilder(LCCBlocks.cracked_mud)
            .addSection(KnowledgeArticleSectionBuilder(introduction)
                .addFragment(KnowledgeArticleTextFragmentBuilder("%s is a block originally introduced in %s and %s and reintroduced in %s that covers the surface of the %s. %s left in the %s without adjacent %s will slowly dry into %s.")
                    .insert(LCCBlocks.cracked_mud.name)
                    .insertLink("YAM Update 1", LCCVersion.YAM_1.page.link)
                    .insertLink("LCC Forge Beta 1.0", LCCVersion.LCC_FORGE_BETA_1_0.page.link)
                    .insertLink("LCC 0.5.0", LCCVersion.LCC_FABRIC_0_5_0.page.link)
                    .insertLink("Wasteland", KnowledgeArticleIdentifier(BuiltinRegistries.BIOME.key.value, LCC.id("wasteland")).link)
                ...
                .addFragment(KnowledgeArticleTextFragmentBuilder("%s can be rehydrated into %s by being placed in or adjacent to %s. %s can be manually dried into %s by smelting it in a %s or %s.")
                    .insert(LCCBlocks.cracked_mud.name)
    ...
    val block_mud by entry(::initialiser) {
        KnowledgeArticleBuilder(LCCBlocks.mud)
            .addSection(KnowledgeArticleSectionBuilder(introduction)
                .addFragment(KnowledgeArticleTextFragmentBuilder("%s is a block originally introduced in %s and reintroduced in %s which greatly impacts the movement of mobs and players walking on it.")
                    .insert(LCCBlocks.mud.name)
                    .insertLink("YAM Update 1", LCCVersion.YAM_1.page.link)
                    .insertLink("LCC 0.5.1", KnowledgeArticleIdentifier(LCC.id("version"), LCC.id("0.5.1")).link)
                )
                .addFragment(KnowledgeArticleTextFragmentBuilder("%s slowly turns into %s when placed in the %s biome without %s adjacent. Alternatively, %s can be dried in a %s or %s. To reverse the drying process, %s can be hydrated by being placed next to %s.")
                    .insert(LCCBlocks.mud.name)
            ...
            .addSection(KnowledgeArticleSectionBuilder("Movement")
                ...
                .addFragment(KnowledgeArticleTextFragmentBuilder("In %s, %s is a ridiculously slippery block, allowing entities walking on it to wildly accelerate and travel huge distances very quickly.")
                    .insert("YAM Update 1")
                    .insert(LCCBlocks.mud.name)
                )
            )
            .addSection(KnowledgeArticleSectionBuilder("1.19")
                .addFragment(KnowledgeArticleTextFragmentBuilder("This block will likely be merged with the Mud Block from Minecraft 1.19 once released. The processes of converting to %s and vice versa is planned to be kept.")
                    .insert(LCCBlocks.cracked_mud.name)
                )
            )
            .addSection(KnowledgeArticleSectionBuilder("Crafting Recipes")
