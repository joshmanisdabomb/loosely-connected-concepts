    ...
    override fun apply(data: DataAccessor, entry: Block) {
        stateVariant(data, entry) {
            coordinate(BlockStateVariantMap.create(WastelandObeliskBlock.charge, Properties.BOTTOM).register { charge, bottom ->
                ...
                if (bottom) BlockStateVariant.create().put(VariantSettings.MODEL, LCCModelTemplates.template_obelisk_bottom.upload(idh.locSuffix(entry, "bottom").suffix(charge.toString()), texture, data.modelStates::addModel))
                else BlockStateVariant.create().put(VariantSettings.MODEL, LCCModelTemplates.template_obelisk_top.upload(idh.locSuffix(entry, "top").suffix(charge.toString()), texture, data.modelStates::addModel))
            })
        }
    }
