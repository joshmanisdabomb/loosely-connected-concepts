    ...
    fun generateTree(context: FeatureContext<TreeFeatureConfig>, r: Int, branches: Map<Direction, IntRange>, blockPos2: BlockPos) {
        ...
        horizontalDirections.forEach {
            for (i in -1..0) {
                ...
                setBlockState(context.world, pos6, context.config.foliageProvider.getBlockState(context.random, pos6).with(Properties.DISTANCE_1_7, 1))
            }
        }
        for (i in 1..2) {
            val pos5 = blockPos2.up(r+i)
            setBlockState(context.world, pos5, context.config.foliageProvider.getBlockState(context.random, pos5).with(Properties.DISTANCE_1_7, i))
        }
            ...
            for (i in v) {
                ...
                horizontalDirections.forEach {
                    if (i >= 0 || (i == -1 && context.random.nextInt(2) == 0) || (flag && v.first > 0 && i == -2 && context.random.nextInt(2) == 0)) {
                        ...
                        if (TreeFeature.canReplace(context.world, pos3)) setBlockState(context.world, pos3, context.config.foliageProvider.getBlockState(context.random, pos3).with(Properties.DISTANCE_1_7, 1))
                    }
                }
                if (i == v.last) {
                    val pos4 = pos2.add(0, 1, 0)
                    if (TreeFeature.canReplace(context.world, pos4)) setBlockState(context.world, pos2.add(0, 1, 0), context.config.foliageProvider.getBlockState(context.random, pos2.offset(k)).with(Properties.DISTANCE_1_7, 1))
                }
            }
        }
    }
