    ...
    override fun apply(data: DataAccessor, entry: Block) {
        stateVariant(data, entry) {
            coordinate(BlockStateVariantMap.create(DungeonTableBlock.ENTITY, Properties.BOTTOM).register { entity, bottom ->
                ...
                else BlockStateVariant.create().put(VariantSettings.MODEL, LCCModelTemplates.template_spawner_table.upload(idh.loc(entry), TextureMap.sideTopBottom(entry).put(TextureKey.PARTICLE, idh.locSuffix(entry, "bottom")), data.models))
            })
        }
    }
