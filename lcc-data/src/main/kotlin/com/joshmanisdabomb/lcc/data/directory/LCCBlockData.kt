...
object LCCBlockData : BasicDirectory<BlockDataContainer, Unit>() {
    ...
    val rubber_leaves by entry(::initialiser) { BlockDataContainer().defaultLang().defaultItemAsset().add(LeavesBlockLootFactory(LCCBlocks.rubber_sapling, saplingChance = floatArrayOf(0.09F, 0.12F, 0.15F, 0.2F))).add(BlockTagFactory(BlockTags.LEAVES)).add(TintBlockAssetFactory).add(BlockTagFactory(BlockTags.LEAVES)).add(ItemTagFactory(ItemTags.LEAVES)).add(ItemTagFactory(LCCTags.rubber_tree)) }
