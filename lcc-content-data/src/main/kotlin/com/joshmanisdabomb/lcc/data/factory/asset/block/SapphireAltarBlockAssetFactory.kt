...
object SapphireAltarBlockAssetFactory : BlockAssetFactory {
    ...
    override fun apply(data: DataAccessor, entry: Block) {
        val altar = LCCModelTemplates.template_sapphire_altar.upload(idh.loc(entry), Texture().put(TextureKey.FRONT, idh.locSuffix(entry, "front")).put(TextureKey.SIDE, idh.loc(entry)).put(TextureKey.INSIDE, idh.locSuffix(entry, "top")).put(TextureKey.BOTTOM, idh.locSuffix(entry, "gems")).put(TextureKey.PARTICLE, idh.loc(entry)), data.modelStates::addModel)
        val key = LCCModelTemplates.template_sapphire_altar_key.upload(idh.locSuffix(entry, "key"), Texture.texture(idh.loc(LCCBlocks.rusted_iron_blocks.values.last())), data.modelStates::addModel)
        stateMultipart(data, entry) {
            horizontalDirections.forEach {
                ...
                SapphireAltarBlock.SapphireState.values().filter { it != SapphireAltarBlock.SapphireState.EMPTY }.forEach { s ->
                    ...
                    val tl = LCCModelTemplates.template_sapphire_altar_tl.upload(idh.locSuffix(entry, "tl").suffix(if (s == SapphireAltarBlock.SapphireState.DULL) "dull" else null), texture, data.modelStates::addModel)
                    val tr = LCCModelTemplates.template_sapphire_altar_tr.upload(idh.locSuffix(entry, "tr").suffix(if (s == SapphireAltarBlock.SapphireState.DULL) "dull" else null), texture, data.modelStates::addModel)
                    val middle = LCCModelTemplates.template_sapphire_altar_middle.upload(idh.locSuffix(entry, "middle").suffix(if (s == SapphireAltarBlock.SapphireState.DULL) "dull" else null), texture, data.modelStates::addModel)
                    val bl = LCCModelTemplates.template_sapphire_altar_bl.upload(idh.locSuffix(entry, "bl").suffix(if (s == SapphireAltarBlock.SapphireState.DULL) "dull" else null), texture, data.modelStates::addModel)
                    val br = LCCModelTemplates.template_sapphire_altar_br.upload(idh.locSuffix(entry, "br").suffix(if (s == SapphireAltarBlock.SapphireState.DULL) "dull" else null), texture, data.modelStates::addModel)
