...
object LCCBlockData : BasicDirectory<BlockDataContainer, Unit>(), ModelAccess {
    ...
    val energy_bank by entry(::initialiser) { data().defaultLang().defaultLootTable().defaultItemAsset().add(DirectionalBlockAssetFactory(model = mb.orientableBottom({ LCCBlocks.machine_enclosure.identifierLoc() }, { idb.loc(it) }, { LCCBlocks.machine_enclosure.identifierLocSuffix("side") }))).add(CustomRecipeFactory { d, i ->
        ShapedRecipeJsonFactory(i, 1)
            .pattern("bib")
            .pattern("bmb")
            .pattern("bib")
    ...
    val treetap_bowl by entry(::initialiser) { data().defaultLang().add(TreetapStorageBlockAssetFactory(AbstractTreetapBlock.TreetapContainer.BOWL, LCCModelTemplates.template_treetap_bowl_1, LCCModelTemplates.template_treetap_bowl_2, LCCModelTemplates.template_treetap_bowl_3, LCCModelTemplates.template_treetap_bowl_dried) { d, t, i -> LCCModelTemplates.template_treetap_bowl.upload(i(t), Texture().put(LCCModelTextureKeys.t0, LCCBlocks.treetap.identifierLoc()).put(LCCModelTextureKeys.t1, idb.loc(t)), d.modelStates::addModel) }).add(TreetapContainerBlockLootFactory(LCCBlocks.treetap, Items.BOWL)) }
    ...
    val classic_cobblestone_slab by entry(::initialiser) { data().defaultLang().defaultItemAsset().add(SlabBlockAssetFactory(LCCBlocks.classic_planks.identifierLoc(), full = LCCBlocks.classic_planks.identifierLoc())).add(BlockTagFactory(BlockTags.SLABS)).add(ItemTagFactory(ItemTags.SLABS)).add(SlabRecipeFactory(LCCBlocks.classic_cobblestone)).add(RiftFromItemRecipeFactory(Blocks.COBBLESTONE_SLAB)).add(SlabLootFactory).add(StonecutterItemRecipeFactory(LCCBlocks.classic_cobblestone, 2)) }
    ...
    val scattered_salt by entry(::initialiser) { data().defaultLang().add(SaltBlockLootFactory).add(SaltBlockAssetFactory).add(CustomItemAssetFactory { d, t, i ->
        LCCModelTemplates.template_salt_item.upload(i(t), Texture.texture(LCC.block("salt")), d.modelStates::addModel)
    }) }
    ...
    val alarm by entry(::initialiser) { data().defaultLang().defaultLootTable().add(CustomItemAssetFactory { d, t, i -> LCCModelTemplates.template_alarm_item.upload(i(t), Texture.texture(idi.loc(t, folder = "block")), d.modelStates::addModel) }).add(HorizontalBlockAssetFactory({ d, t, i -> LCCModelTemplates.template_alarm.upload(i(t), Texture.texture(idb.loc(t)), d.modelStates::addModel) })).add(CustomRecipeFactory { d, i ->
        ShapedRecipeJsonFactory(i, 2)
            .pattern("bib")
            .pattern("nln")
            .input('b', Blocks.BELL)
