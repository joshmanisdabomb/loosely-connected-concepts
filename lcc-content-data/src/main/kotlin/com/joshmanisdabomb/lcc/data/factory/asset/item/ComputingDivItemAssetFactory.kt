...
class ComputingDivItemAssetFactory(val levels: IntArray, val model: (key: Int) -> ModelProvider.ModelFactory<Item>) : ItemAssetFactory {

    override fun apply(data: DataAccessor, entry: Item) {
        val item = entry as? ComputingItem ?: return

        val ids = levels.mapIndexed { k, v -> v.toFloat().minus(item.initialLevel).div(item.maxLevel.minus(item.initialLevel)) to model(k+1).create(data, entry) { idh.locSuffix(it, if (k != 0) (k+1).toString() else null) } }
